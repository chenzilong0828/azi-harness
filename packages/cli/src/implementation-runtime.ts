...
function createContextMarkdown(input: {
  ...
  return [
    ...
    `- Figma 缓存状态：${input.figma === null ? "未生成" : `${input.figma.cacheReuse.status} (${input.figma.cacheReuse.cacheKey})`}`,
    `- Figma 命中路径：${input.figma?.cacheReuse.matchedCachePath ?? "无"}`,
    `- 来源补丁：${input.figma?.proposalPath ?? "未生成"}`,
    `- 来源缓存说明：${input.figma?.cacheReuse.message ?? "无"}`,
